/*
    Kaleidoscope, version [unreleased]. Copyright 2025 Jon Pretty, Propensive OÜ.
  ...
  def run(): Unit =
    suite(t"Regex tests"):
      ...
      suite(t"Standard parsing"):
        ...
        test(t"Parse (aaa)")(Regex.parse(List(t"(aaa)"))).assert:
          _ == Regex(t"(aaa)", List(Group(1, 4, 5)))

        test(t"Parse (aa)bb")(Regex.parse(List(t"(aa)bb"))).assert:
          _ == Regex(t"(aa)bb", List(Group(1, 3, 4)))

        test(t"Parse aa(bb)")(Regex.parse(List(t"aa(bb)"))).assert:
          _ == Regex(t"aa(bb)", List(Group(3, 5, 6)))

        test(t"Parse aa(bb)cc")(Regex.parse(List(t"aa(bb)cc"))).assert:
          _ == Regex(t"aa(bb)cc", List(Group(3, 5, 6)))

        test(t"Parse aa(bb)+?cc")(Regex.parse(List(t"aa(bb)+?cc"))).assert:
          _ == Regex(t"aa(bb)+?cc", List(Group(3, 5, 8, Nil, AtLeast(1), Reluctant)))

        test(t"Parse aa(bb)++cc")(Regex.parse(List(t"aa(bb)++cc"))).assert:
          _ == Regex(t"aa(bb)++cc", List(Group(3, 5, 8, Nil, AtLeast(1), Possessive)))

        test(t"Parse aa(bb)cc(dd)ee")(Regex.parse(List(t"aa(bb)cc(dd)ee"))).assert:
          _ == Regex(t"aa(bb)cc(dd)ee", List(Group(3, 5, 6), Group(9, 11, 12)))

        test(t"Parse aa(bb(cc)dd)ee")(Regex.parse(List(t"aa(bb(cc)dd)ee"))).assert:
          _ == Regex(t"aa(bb(cc)dd)ee", List(Group(3, 11, 12, List(Group(6, 8, 9)))))

        test(t"Parse aa(bb)*cc(dd)ee")(Regex.parse(List(t"aa(bb)*cc(dd)ee"))).assert:
          _ == Regex(t"aa(bb)*cc(dd)ee", List(Group(3, 5, 7, Nil, AtLeast(0)), Group(10, 12, 13)))
      ...
      suite(t"Parsing failures"):
        ...
        .assert(_ == RegexError(RegexError.Reason.UnexpectedChar))
        ...
        test(t"Fail to parse aa(bb{14}ccddee"):
          capture(Regex.parse(List(t"aa(bb{14}ccddee")))
        .assert(_ == RegexError(RegexError.Reason.UnclosedGroup))

        test(t"Fail to parse aa(bb){2,1}c"):
          capture(Regex.parse(List(t"aa(bb){2,1}c")))
        .assert(_ == RegexError(RegexError.Reason.BadRepetition))

        test(t"Fail to parse aabb){2,1}c"):
          capture(Regex.parse(List(t"aabb){2,1}c")))
        .assert(_ == RegexError(RegexError.Reason.NotInGroup))
        ...
        .assert(_ == RegexError(RegexError.Reason.IncompleteRepetition))

      suite(t"Test captures"):
        test(t"Capture without parens should fail"):
          capture(Regex.parse(List(t"a", t"a(bb)")))
        .assert(_ == RegexError(RegexError.Reason.ExpectedGroup))
        ...
        .assert(_ == RegexError(RegexError.Reason.Uncapturable))
